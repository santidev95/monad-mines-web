...
export default function Home() {
  ...
  const renderGrid = () => {
      ...
      for (let x = 0; x < 10; x++) {
        ...
        cells.push(
            <button 
              ...
              className={`
                w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 border-2 rounded-lg
                ${cellClass}
                ...
                flex items-center justify-center text-sm sm:text-base md:text-xl touch-manipulation
              `}
            >
              {cellContent}
            </button>
    ...
    return (
      <div className={`grid grid-cols-10 gap-1 sm:gap-1.5 md:gap-2 p-2 sm:p-3 md:p-4 bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-gray-700/50 ${gridShake ? 'animate-shake' : ''}`}>
        {cells}
      </div>
    )
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900 text-white p-2 sm:p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-4xl">
          ...
          <div className="text-center mb-4 relative z-20">
            <button
              ...
              className="absolute top-0 right-0 z-30 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg font-semibold text-xs sm:text-sm transition-all transform hover:scale-105 shadow-md cursor-pointer pointer-events-auto touch-manipulation"
            >
              ❓ How to Play
            </button>
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-black mb-1 bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent drop-shadow-2xl px-8 sm:px-0">
              💎 MONAD MINES 💎
            </h1>
            <p className="text-gray-400 text-[10px] sm:text-xs mt-1">Discover the mines and win the pot!</p>
          </div>
          ...
          {!isConnected ? (
            ...
            <div className="flex flex-col gap-3">
                {/* Status Bar */}
                <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-sm p-2 sm:p-3 rounded-xl border border-gray-700/50 shadow-xl flex flex-col gap-2 sm:gap-3">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3">
                        <div className="space-y-1.5 sm:space-y-2 w-full sm:w-auto">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                              <span className="text-gray-400 text-[10px] sm:text-xs">Main Wallet:</span>
                              <span className="text-purple-300 font-mono text-xs sm:text-sm font-semibold break-all sm:break-normal">{address?.slice(0,6)}...{address?.slice(-4)}</span>
                            </div>
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                              <span className="text-gray-500 text-[10px] sm:text-xs">Balance:</span>
                              <span className="text-purple-400 font-mono text-xs sm:text-sm font-semibold">{parseFloat(mainBalance).toFixed(4)} MON</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-gray-400 text-[10px] sm:text-xs">Session:</span>
                              {isSessionActive ? (
                                <span className="flex items-center gap-1 text-green-400 font-semibold text-xs sm:text-sm">
                                  <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-400 rounded-full animate-pulse"></span>
                                  Active
                                </span>
                              ) : (
                                <span className="flex items-center gap-1 text-red-400 font-semibold text-xs sm:text-sm">
                        ...
                        <div className="text-left sm:text-right space-y-1.5 sm:space-y-2 w-full sm:w-auto">
                            <div className="space-y-1">
                              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                <span className="text-gray-400 text-xs sm:text-sm">Game Wallet:</span>
                                <span className="text-yellow-400 font-mono font-bold text-base sm:text-lg">{parseFloat(burnerBalance).toFixed(4)} MON</span>
                              </div>
                              {burnerAccount && (
                                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                  <span className="text-gray-400 text-[10px] sm:text-xs">Address:</span>
                                  <span className="text-purple-300 font-mono text-[10px] sm:text-xs break-all sm:break-normal">{burnerAccount.address.slice(0,6)}...{burnerAccount.address.slice(-4)}</span>
                                </div>
                              )}
                            </div>
                            <div className="flex gap-2">
                              <button 
                                ...
                                className="flex-1 sm:flex-none text-xs bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 px-2 sm:px-3 py-1.5 rounded-lg font-semibold transition-all transform hover:scale-105 shadow-md touch-manipulation"
                              >
                                ⛽ Top Up
                              </button>
                              {parseFloat(burnerBalance) > 0.001 && (
                                <button 
                                  ...
                                  className="flex-1 sm:flex-none text-xs bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 px-2 sm:px-3 py-1.5 rounded-lg font-semibold transition-all transform hover:scale-105 shadow-md touch-manipulation"
                                >
                                  💸 Withdraw
                                </button>
                              )}
                ...
                {!isSessionActive ? (
                    <button 
                      ...
                      className="w-full bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-500 hover:to-orange-500 py-2.5 sm:py-2 rounded-xl font-bold text-xs sm:text-sm transition-all transform hover:scale-105 shadow-lg shadow-yellow-500/30 touch-manipulation"
                    >
                      🔑 Authorize Session
                    </button>
                ) : (
                    <button 
                      ...
                      className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 py-2.5 sm:py-2 rounded-xl font-bold text-xs sm:text-sm transition-all transform hover:scale-105 shadow-lg shadow-red-500/30 touch-manipulation"
                    >
                      🚫 Revoke Session
                    </button>
                )}
                ...
                <div className="flex flex-col lg:flex-row gap-3">
                    {/* Logs - Left Side */}
                    <div className="lg:w-72 bg-gradient-to-br from-black/80 to-gray-900/80 backdrop-blur-sm h-48 sm:h-64 lg:h-auto overflow-y-auto text-[10px] sm:text-xs text-green-400 p-2 sm:p-3 font-mono border border-gray-800 rounded-xl shadow-xl order-2 lg:order-1">
                    ...
                    <div className="flex-1 bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-sm p-2 sm:p-4 md:p-5 rounded-2xl border border-gray-700/50 shadow-2xl order-1 lg:order-2">
                        {/* Game Header - Compact */}
                        <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
                            ...
                            {(game.status === 'idle' || game.status === 'game_over' || game.status === 'won') && (
                              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded-lg px-2 sm:px-3 py-2 backdrop-blur-sm w-full sm:w-auto">
                                ...
                                <input
                                  ...
                                  className="w-full sm:w-20 bg-gray-800/50 border border-gray-700 rounded px-2 py-1.5 sm:py-1 text-white text-xs sm:text-sm font-mono focus:outline-none focus:border-purple-500 transition-all"
                                  placeholder="0.001"
                                />
                                <div className="flex gap-1 w-full sm:w-auto">
                                  <button
                                    onClick={() => setBetAmount("0.001")}
                                    className="flex-1 sm:flex-none bg-gray-700 hover:bg-gray-600 px-2 py-1 rounded text-xs transition-all touch-manipulation"
                                  >
                                    0.001
                                  </button>
                                  <button
                            ...
                            <div className="flex gap-2 items-center w-full sm:w-auto justify-between sm:justify-start">
                              <div className="bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 border border-yellow-500/50 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 backdrop-blur-sm flex-1 sm:flex-none">
                                <p className="text-gray-400 text-[10px] sm:text-xs">Pot</p>
                                <span className="text-base sm:text-xl text-yellow-400 font-bold font-mono">
                                  {game.pot} MON
                                </span>
                              </div>
                              {game.status === 'playing' && (
                                <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/50 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 backdrop-blur-sm flex-1 sm:flex-none">
                                  <p className="text-gray-400 text-[10px] sm:text-xs">Mult</p>
                                  <span className="text-base sm:text-xl text-green-400 font-bold font-mono">
                                    {currentMultiplier}x
                                  </span>
                                </div>
                              )}
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                              {game.status === 'playing' && (
                                <button 
                                  ...
                                  className="flex-1 sm:flex-none bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 px-3 sm:px-4 py-2 rounded-lg font-bold text-xs sm:text-sm text-white shadow-lg shadow-green-500/30 transition-all transform hover:scale-105 touch-manipulation"
                                >
                                  💰 CASH OUT
                                </button>
                              )}
                              {(game.status === 'idle' || game.status === 'game_over' || game.status === 'won') && (
                                <button 
                                  onClick={startGame} 
                                  className="flex-1 sm:flex-none bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 px-3 sm:px-4 py-2 rounded-lg font-bold text-xs sm:text-sm text-white shadow-lg shadow-purple-500/30 transition-all transform hover:scale-105 touch-manipulation"
                                >
                                  🎮 NEW GAME
                                </button>
                              )}
                          ...
                          {game.status === 'waiting_pyth' ? (
                            ...
                            <>
                              {game.status === 'waiting_nonce' && (
                                <div className="mb-3 sm:mb-4 text-center w-full">
                                  <div className="animate-pulse text-xl sm:text-2xl mb-2">🔐</div>
                                  <p className="text-xs sm:text-sm text-yellow-300 font-semibold px-2">Game ready! Click on any cell to reveal the nonce.</p>
                                </div>
                              )}
        ...
        {/* Top Up Modal */}
        {showTopUpModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
            <div className="bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-md border-2 border-purple-500/50 rounded-2xl p-4 sm:p-6 max-w-md w-full mx-4 shadow-2xl animate-bounceIn max-h-[95vh] overflow-y-auto">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent mb-4 sm:mb-6 text-center">
                ⛽ Top Up Burner Account
              </h2>
              
              <div className="mb-4 sm:mb-6 bg-gradient-to-r from-yellow-500/10 to-yellow-600/10 border border-yellow-500/30 rounded-xl p-3 sm:p-4">
                <p className="text-gray-400 text-xs sm:text-sm mb-2">💰 Current balance in your wallet:</p>
                <p className="text-xl sm:text-2xl font-mono text-yellow-400 font-bold">{parseFloat(mainBalance).toFixed(4)} MON</p>
              </div>

              <div className="mb-4 sm:mb-6">
                <label className="block text-gray-300 text-xs sm:text-sm mb-2 sm:mb-3 font-semibold">
                ...
                <input
                  ...
                  className="w-full bg-gray-800/50 border-2 border-gray-700 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-white text-base sm:text-lg font-mono focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/50 transition-all"
                  placeholder="0.05"
                />
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3">
                  <button
                    onClick={() => setTopUpAmount("0.01")}
                    className="bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all transform hover:scale-105 touch-manipulation"
                  >
                    0.01
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all transform hover:scale-105 touch-manipulation"
                  >
                    Max
                  </button>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <button
                  onClick={handleTopUp}
                  className="flex-1 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-base sm:text-lg transition-all transform hover:scale-105 shadow-lg shadow-purple-500/30 touch-manipulation"
                >
                  ✅ Confirm
                </button>
                <button
                  onClick={() => setShowTopUpModal(false)}
                  className="flex-1 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-base sm:text-lg transition-all transform hover:scale-105 touch-manipulation"
                >
                  ❌ Cancel
                </button>
              </div>
        ...
        {showWithdrawModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
            <div className="bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-md border-2 border-green-500/50 rounded-2xl p-4 sm:p-6 max-w-md w-full mx-4 shadow-2xl animate-bounceIn max-h-[95vh] overflow-y-auto">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent mb-4 sm:mb-6 text-center">
                💸 Withdraw
              </h2>
              
              <div className="mb-4 space-y-2 sm:space-y-3">
                <div className="bg-gradient-to-r from-yellow-500/10 to-yellow-600/10 border border-yellow-500/30 rounded-xl p-3 sm:p-4">
                  <p className="text-gray-400 text-xs sm:text-sm mb-2">💰 Current balance in burner account:</p>
                  <p className="text-xl sm:text-2xl font-mono text-yellow-400 font-bold">{parseFloat(burnerBalance).toFixed(6)} MON</p>
                </div>
                <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-xl p-2 sm:p-3">
                  <p className="text-gray-400 text-[10px] sm:text-xs mb-1">⛽ Estimated gas:</p>
                  <p className="text-base sm:text-lg font-mono text-blue-400 font-semibold">{parseFloat(estimatedGas).toFixed(6)} MON</p>
                </div>
                <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-2 sm:p-3">
                  <p className="text-gray-400 text-[10px] sm:text-xs mb-1">✅ Available to withdraw:</p>
                  <p className="text-base sm:text-lg font-mono text-green-400 font-semibold">
              ...
              <div className="mb-4 sm:mb-6">
                ...
                <input
                  ...
                  className="w-full bg-gray-800/50 border-2 border-gray-700 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-white text-base sm:text-lg font-mono focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/50 transition-all"
                  placeholder="0"
                />
                <div className="flex flex-col sm:flex-row gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none touch-manipulation"
                  >
                    Max (All - Gas)
                  </button>
                  <button
                    ...
                    disabled={isWithdrawing}
                    className="flex-1 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 px-3 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none touch-manipulation"
                  >
                    50%
                  </button>
                  <button
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <button
                  ...
                  disabled={isWithdrawing}
                  className="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-base sm:text-lg transition-all transform hover:scale-105 shadow-lg shadow-green-500/30 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2 touch-manipulation"
                >
                  {isWithdrawing ? (
                    <>
                ...
                <button
                  ...
                  disabled={isWithdrawing}
                  className="flex-1 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-base sm:text-lg transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none touch-manipulation"
                >
                  ❌ Cancel
                </button>
              </div>
        ...
        {showHowToPlayModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
            <div className="bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-md border-2 border-purple-500/50 rounded-2xl p-4 sm:p-6 max-w-2xl w-full mx-4 shadow-2xl animate-bounceIn max-h-[95vh] overflow-y-auto">
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent mb-4 sm:mb-6 text-center">
                📖 How to Play
              </h2>
              ...
              <div className="mt-4 sm:mt-6 flex justify-center">
                <button
                  ...
                  className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold text-base sm:text-lg transition-all transform hover:scale-105 shadow-lg shadow-purple-500/30 touch-manipulation"
                >
                  Got it!
                </button>
              </div>
        ...
        {showMineModal && (
          <div className="fixed inset-0 bg-black bg-opacity-95 flex items-center justify-center z-50 animate-fadeIn p-2 sm:p-4">
            <div className="relative bg-gradient-to-br from-red-900 via-red-800 to-orange-900 border-4 border-red-500 rounded-lg p-4 sm:p-6 md:p-8 max-w-md w-full mx-4 text-center animate-bounceIn shadow-2xl shadow-red-900">
              ...
              <div className="relative mb-4 sm:mb-6">
                <div className="text-6xl sm:text-8xl md:text-9xl mb-3 sm:mb-4 animate-pulse drop-shadow-2xl">💣</div>
                <div className="flex items-center justify-center gap-2 mb-2">
                  <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white animate-pulse drop-shadow-lg">
                    BOOM!
                  </h2>
                </div>
                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-yellow-300 mb-3 sm:mb-4 drop-shadow-lg">
                  💥 MINE FOUND! 💥
                </h3>
                {minePosition && (
                ...
                <div className="bg-black/50 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4 border-2 border-yellow-500/50">
                  <p className="text-xs sm:text-sm text-yellow-200 mb-2">Lost Pot:</p>
                  <p className="text-xl sm:text-2xl md:text-3xl text-yellow-400 font-mono font-bold">
                    {lostPot} MON
                  </p>
                </div>
              ...
              <button
                ...
                className="relative w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 px-4 sm:px-6 py-3 sm:py-4 rounded-lg font-bold text-white text-base sm:text-lg transition-all transform hover:scale-105 shadow-lg shadow-red-900/50 border-2 border-red-400 touch-manipulation"
              >
                Got it - Close
              </button>
            </div>
